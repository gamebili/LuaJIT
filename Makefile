...
LUA54COMPAT_HOST_SYS:= $(shell uname -s 2>/dev/null)
ifeq ($(LUA54COMPAT_HOST_SYS),Darwin)
ifneq (,$(filter arm64 aarch64,$(LUA54COMPAT_HOST_ARCH)))
  LUA54COMPAT_XCFLAGS+= -DLUAJIT_UNWIND_INTERNAL
endif
endif
...
run-lua54compat-capi-perf-tests: run-lua54compat-tests run-capi-lua54compat-tests run-perf-lua54compat-tests
