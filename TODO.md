...
- [ ] Lua 5.4 C API / 头文件兼容。
  ...
  - 当前进展：Lua 5.4 外部 `lualib.h` 已隐藏 LuaJIT 专有 `LUA_BITLIBNAME` / `LUA_JITLIBNAME` / `LUA_FFILIBNAME`、`luaopen_bit` / `luaopen_jit` / `luaopen_ffi` / `luaopen_string_buffer`，并在单独包含 `lualib.h` 时不再暴露 lauxlib-only 的 `LUA_GNAME` / `LUA_FILEHANDLE`；默认构建和内部 ABI 继续保留这些 LuaJIT 扩展。
