...
- [ ] Lua 5.4 auxiliary library / lauxlib 兼容。
  ...
  - 已覆盖：`luaL_checkinteger()` / `luaL_optinteger()` 的 fraction number 错误，`lua_tointegerx()` 的 fraction status，`luaL_intop()` 的 32 位公开整数范围 wraparound，以及上述常用 lauxlib 5.4 辅助入口的编译/链接/运行表面。
