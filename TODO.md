...
- [ ] Lua 5.4 C API / 头文件兼容。
  ...
  - 当前进展：Lua 5.4 外部兼容头已把 `lua_getextraspace()` 暴露为宏，并映射到当前 pointer-sized `L->exdata` 兼容存储；`lua_newstate()` / `lua_getallocf()` / `lua_setallocf()` 自定义 allocator 表面已进入 C API smoke；默认构建仍保留 LuaJIT 旧函数 ABI。
