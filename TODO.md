...
- [ ] `debug.getinfo` 的 Lua 5.4 选项和 hook 字段。
  ...
  - 当前进展：普通 Lua `BC_CALLT` 路径已在 x64 / ARM64 VM 中保存 side marker，`debug.getinfo(..., "t")`、Lua hook 和 C API hook 都能识别 `tail call` / `LUA_HOOKTAILCALL`，并在该 frame 返回时清理 marker，避免后续普通调用误判。
  ...
  - 已覆盖：C API smoke 读取新增 `lua_Debug` 字段，并通过 `lua_sethook` / `lua_getinfo(..., "r")` 覆盖普通 Lua 函数固定参数和 vararg 的 hook transfer、Lua tail call hook transfer / `istailcall`，以及普通 C 函数 call/return hook transfer；Lua smoke 覆盖非 hook 场景和 `debug.getinfo(2, "r")` 的固定参数 / vararg call-return hook transfer、C 函数 call hook transfer、fast C 函数 return hook transfer、Lua tail call hook/return transfer 和直接调用清 marker。
  - 需要补测试：多层嵌套 tail call marker、tail call 错误展开清理、tail-position C return 和更多 hook transfer 字段边界。
