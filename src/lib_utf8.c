...
static int utf8_char(lua_State *L)
{
  ...
  luaL_buffinit(L, &b);
  for (i = 1; i <= n; i++)
    utf8_addchar(L, &b, luaL_checkinteger(L, i));
  luaL_pushresult(&b);
  return 1;
}
...
static int utf8_codepoint(lua_State *L)
{
  ...
  lua_Integer i = utf8_posrelat(luaL_optinteger(L, 2, 1), len);
  lua_Integer j = utf8_posrelat(luaL_optinteger(L, 3, i), len);
  int strict = !lua_toboolean(L, 4);
  int n = 0;
  uint32_t cp;
  if (i < 1 || j < 0 || i > (lua_Integer)len + 1)
    luaL_argerror(L, 2, "position out of range");
...
static int utf8_len(lua_State *L)
{
  ...
  const unsigned char *s = (const unsigned char *)luaL_checklstring(L, 1, &len);
  lua_Integer i = utf8_posrelat(luaL_optinteger(L, 2, 1), len);
  lua_Integer j = utf8_posrelat(luaL_optinteger(L, 3, -1), len);
  lua_Integer n = 0;
  int strict = !lua_toboolean(L, 4);
  uint32_t cp;
...
static int utf8_offset(lua_State *L)
{
  ...
  lua_Integer ipos = lua_isnoneornil(L, 3) ?
    ...
    utf8_posrelat(luaL_checkinteger(L, 3), len);
  uint32_t cp;
