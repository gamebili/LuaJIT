...
local function package_helpers(n)
  ...
  for _ = 1, n do
    ...
    found, searcherr = package.searchpath("a.b", "?.lua", ".", nil)
    if found == nil and
       searcherr:find("a"..package.config:sub(1, 1).."b.lua", 1, true) then
      sum = sum + 1
    end

    found, searcherr = package.searchpath("a.b", "?.lua", ".", "")
    if found == nil and searcherr == "no file 'ab.lua'" then
      sum = sum + 1
    end

    found, searcherr = package.searchpath("a.b", "?.lua", "", "/")
    if found == nil and searcherr == "no file 'a.b.lua'" then
      sum = sum + 1
    end
