...
- 已按官方 Lua 5.4.8 `lua.h` / `lapi.c` 对齐外部 registry/upvalue 伪索引：外部 `LUA_REGISTRYINDEX` 和 `lua_upvalueindex()` 现在使用官方公式，运行时会接受这些官方伪索引并映射回 LuaJIT 内部 registry/upvalue ABI，默认构建保持旧值。
